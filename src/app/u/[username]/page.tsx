...
export default function PublicPortfolioPage() {
  ...
  const fetchPortfolio = async () => {
    try {
      ...
      if (!res.ok) {
        if (res.status === 401) throw new Error("Please login to view your profile.");
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="md:pl-64 flex flex-col min-h-screen w-full">
        ...
        <main className="p-6 md:p-12 max-w-7xl mx-auto w-full space-y-16">
            ...
            <div className="lg:col-span-8 glass-card rounded-xl p-6 md:p-8 flex flex-col md:flex-row gap-8 items-center md:items-start">
              ...
              <div className="flex-1 space-y-4 text-center md:text-left">
                <div className="space-y-1">
                  <h1 className="text-4xl md:text-5xl font-bold text-text-primary tracking-tight">{data.user?.name}</h1>
                ...
                <div className="flex flex-wrap justify-center md:justify-start gap-3">
                  <div className="flex items-center gap-2 bg-white dark:bg-slate-900 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-800">
                    <span className="material-symbols-outlined text-text-secondary text-lg">hub</span>
                    <span className="text-xs font-bold text-text-secondary">@{data.user?.githubUsername}</span>
                  </div>
              ...
              <div>
                ...
                <div className="relative flex items-center justify-center">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <span className="text-[10px] font-bold text-text-secondary uppercase tracking-widest mt-1">Elite</span>
                  </div>
                </div>
              </div>
              <div className="mt-8 space-y-2">
                <p className="text-xs font-medium text-text-secondary">Top tier engineering alignment detected based on patterns.</p>
                ...
                <a key={repo.id} href={repo.url} target="_blank" rel="noreferrer" className="group glass-card overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800 hover:border-indigo-500/50 transition-all flex flex-col">
                  <div className="p-6 space-y-4">
                    <div className="flex justify-between items-start">
                      <h3 className="text-xl font-bold text-text-primary group-hover:text-indigo-600 transition-colors">{repo.name}</h3>
                      <div className="flex gap-2">
                        <span className="flex items-center gap-1 text-text-secondary text-xs font-bold">
                    ...
                    <p className="text-sm text-text-secondary line-clamp-2 leading-relaxed h-10">
                    ...
                    <div className="flex gap-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                      ...
                      <span className="text-xs font-bold text-text-secondary ml-auto uppercase tracking-wider">Public Repo</span>
                    </div>
                  </div>
                </a>
              ))}
          ...
          {/* Precision Analysis */}
          <section className="space-y-6">
            <h2 className="text-2xl font-bold text-text-primary">Precision Analysis</h2>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="md:col-span-2 glass-card rounded-xl p-8 space-y-6">
                <h3 className="text-[10px] font-black text-text-secondary uppercase tracking-[0.2em]">Commit Velocity</h3>
                ...
                <p className="text-xs font-medium text-text-secondary">Daily activity patterns show consistent engagement and release peaks.</p>
              </div>
              <div className="glass-card rounded-xl p-8 flex flex-col justify-between text-center items-center">
                ...
                <div>
                  <div className="text-5xl font-black text-text-primary">99%</div>
                  <div className="text-[10px] font-black text-text-secondary uppercase tracking-[0.2em] mt-2">Uptime Score</div>
                </div>
              </div>
              <div className="glass-card rounded-xl p-8 flex flex-col justify-between text-center items-center border-l-4 border-amber-500">
                ...
                <div>
                  <div className="text-5xl font-black text-text-primary">0.4</div>
                  <div className="text-[10px] font-black text-text-secondary uppercase tracking-[0.2em] mt-2">Debt Ratio</div>
                </div>
              </div>
            </div>
          </section>
